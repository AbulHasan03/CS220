...
# [1] Define a function master_theorem(a, b, c) that determines the order of magnitude of the solution to a recurrence
# of the form T(n) = aT(n/b) + O(nc).
# See https://www.geeksforgeeks.org/how-to-analyse-complexity-of-recurrence-relation/ .
def master_theorem(a, b, c):
    ...
    return result
# [2] Define a function parse_recurrence(recurrence) that parses a string representation 
# of a recurrence and extracts the a. b, and c to pass into the function of the previous task. You can make any reasonable 
...
# [4] Define a sample function f1, like the one for MergeSort. Try to use the one-line 
# if/else (aka "ternary expression") as it will make it easier to capture the function content.
def f1(f, n):
    return 0 if n == 1 else 2 * ff(f, n/2) + n


# [5] Create additional functions f2 thru f10 corresponding to nine more recurrences from the slides
# or old exams. 
...
# Time Complexity for Straussen Multiplication, which is diving a matrix into four quadrants
# which makes 7 sub-problems which is n^2
def f3(f, n):
    return 1 if n == 1 else 7 * ff(f, n / 2) + n ** 2

# Time Complexity for D and C (Divide and Conquer) Long Integer Multiplication
def f4(f, n):
...
#Stooge Sort sorts 2/3s at a time
def f11(f,n):
...
def run_functions(n):
    ...
    run_function(functions, data, "Binary Search", "1f(n/2)+1", f2, n)
    ...
    run_function(functions, data, "Teitelman Exam Problem", "16f(n/2)+n", f14, n)
...
def compute_values(funcs, inputs):
    ...
    for func in funcs:
        ...
        dict_f[funcName] = {}
        for n in inputs:
            dict_f[funcName][n] = math.log(func(func, n))
