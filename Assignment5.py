...
# [1] Copy the function from Assignment #1 that gives the content of a function, i.e.
# the code just after the return statement.
def func_body(f):
...
# [2] Define several different functions called f0, f1, f2, f3, etc. Let f0 be the Identity Function,
# i.e. f(n) = n. Let f1 thru f10 be the functions that appear in Slide 50 of Chapter 3.  Then define functions
# f11-f20 using the functions from the Final Exam for Summer 2023, Session 2, Problem 3.
def f0(n):
    return n
...
def compute_values(funcs, inputs):
    dict_funcs = {}
    for func in funcs:
        funcName = func.__name__ + " " + func_body(func)
        dict_funcs[funcName] = {}
        for n in inputs:
