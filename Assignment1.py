...
# [3] Define a function for each expression that needs to be evaluated.
# For example, for the Boolean expression "(p ∨ q) ∧ r", you might write:
def f0(p, q, r):
    return (p or q) and r
...
# [5] Define a function truth_table(f) that creates a truth table for a logical function f.
# Add a comment acknowledging this source:
...
# [6] Define a function analyze_truth_table(f) that calls the previous function to generate the truth table,
# including the evaluated expression, followed by your categorization of the expression as a tautology, contingency,
# or contradiction.
...
# https://en.wikipedia.org/wiki/Truth_table#Truth_table_for_all_binary_logical_operators. Label them f0 thru f15,
# to match the order in Wikipedia. All sixteen should have two propositional variables p and q, even if the function
# uses only one or the other. Verify that the output matches what is expected.
def f0_false(p, q):
    return False
...
# [10] Define a function is_equivalent(f1, f2) that determines if two functions are logically equivalent by building the
# truth tables for both and comparing outputs,or by evaluating f1 ↔ f2 to be a tautology.Test that it works as expected.
def is_equivalent(f1, f2):
    ...
    return result1[0] == result2[0] and result1[1] == result2[1] and all(
        [result1[4][i] == result2[4][i] for i in range(result1[0])])
...
# Returns not only the type (tautology / contingency / contradiction) but also any other data of interest: rows, columns
# variables, output column, binary equivalent, and decimal equivalent
# Define a function is_equivalent(f1, f2) that determines if two functions are logically equivalent by building
# the truth tables for both and comparing outputs, or by evaluating f1 ↔ f2 to be a tautology.
...
def main():
    ...
    for func in functions:
        results = analyze_truth_table(func)
        print(func.__name__, results[5], results[6])
